...
export const DEFAULT_EVENT_MAPPINGS: EventMapping[] = [
  { event: 'erp.invoice.created', actions: ['transform_validate'], enabled: true },
  { event: 'erp.invoice.updated', actions: ['transform_validate'], enabled: true },
]
...
interface SearchableSelectProps {
  ...
  options: { value: string; label: string; description?: string; category?: string }[]
  placeholder: string
  disabled?: boolean
  loading?: boolean
}

function SearchableSelect({ value, onSelect, options, placeholder, disabled, loading }: SearchableSelectProps) {
      ...
      {values.length > 0 && !disabled && (
        <div className="flex flex-wrap gap-1">
          {values.map((v, idx) => {
            ...
            const label = options.find((o) => o.value === strVal)?.label || strVal
            return (
              <Badge key={`${strVal}-${idx}`} variant="secondary" className="text-xs py-0.5 pr-1">
                {label}
                <button
...
export function EventMappingEditor({
  ...
  const [eventTypes, setEventTypes] = useState<EventTypeOption[]>(cachedEventTypes || FALLBACK_EVENT_TYPES)
  const [workflows, setWorkflows] = useState<WorkflowOption[]>(cachedWorkflows || FALLBACK_WORKFLOWS)
    ...
    const fetchReferenceData = async () => {
      try {
        ...
        if (!eventsRes.error && eventsRes.data?.data?.events) {
          const mapped: EventTypeOption[] = eventsRes.data.data.events.map((e: any) => ({
            value: e.id,
            label: e.name,
        ...
        if (!workflowsRes.error && workflowsRes.data?.data?.actions) {
          const mapped: WorkflowOption[] = workflowsRes.data.data.actions.map((a: any) => ({
            value: a.id,
            label: a.name,
  ...
  const resolveEventId = (value: any): string =>
    typeof value === 'object' && value !== null ? (value.id || value.value || String(value)) : String(value || '')
  ...
  const activeRoutes = summarySource.filter((m) => m.event && m.actions.length > 0 && m.enabled)
  ...
  return (
    <div>
      {mappings.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="text-sm text-muted-foreground">
            No event mappings configured.{!readOnly && ' Add a mapping to route incoming events to workflows.'}
          </p>
          {!readOnly && (
            <Button variant="outline" size="sm" onClick={addMapping} className="mt-3" disabled={loading}>
            ...
            <div className="flex flex-wrap gap-2">
              {activeRoutes.map((m, i) => (
                <Badge key={i} variant="secondary" className="text-xs py-1 px-3">
                  {localGetEventLabel(m.event)} → {m.actions.map(a => localGetWorkflowLabel(a)).join(', ')}
                </Badge>
              ))}
            </div>
          </div>
      ...
      <AlertDialog open={deleteIndex !== null} onOpenChange={(open) => { if (!open && !deleting) setDeleteIndex(null) }}>
        <AlertDialogContent>
          <AlertDialogHeader>
